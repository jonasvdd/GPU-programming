...
#define FILTER_FACTOR           1.0/16  // the factor which will be added to the filter result
...
Image *Filters::Blur::Filter(Image *sourceImageOne) {
    ...
    const unsigned char FILTER[BLOCK_WIDTH][FILTER_WIDTH] = {{1, 2, 1},
                                                                         {2, 4, 2},
                                                                         {1, 2, 1}};
            ...
            if ((x >= FILTER_WIDTH / 2) && (x < (imgWidth - FILTER_WIDTH / 2))) {
                ...
                if ((y >= BLOCK_WIDTH / 2) && (y < (imgHeight - BLOCK_WIDTH / 2))) {
                    ...
                    for (unsigned char i = 0; i < NUMB_COLORS; i++) {
                        ...
                        pixelDataDestination[(y * imgWidth + x) * 4 + i] = FILTER_FACTOR * result;
                    }
                    ...
                    for (unsigned char i = 0; i < NUMB_COLORS; i++) {
                        ...
                        for (unsigned char j = 0; j < FILTER_WIDTH; j++)
                            for (unsigned char k = 0; k < BLOCK_WIDTH; k++)
                                result += FILTER[j][k] * sourcePixData[((y - (2 + j) / 2) * imgWidth + x - 1 + k) * 4 + i];
                        pixelDataDestination[(y * imgWidth + x) * 4 + i] = FILTER_FACTOR * result;
                    }
                }
